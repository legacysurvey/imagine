...
config_queries = {
    'default': {
        'from_clause': "candidate left join decam on candidate.id = decam.cand_id left join wise on candidate.id = wise.cand_id left join decam_aper on candidate.id = decam_aper.cand_id",
        'fields': [
            'candidate.id',
            'decam.cand_id',
            'wise.cand_id',
            ...
            for expression in expressions:
                ...
                for m in range(0, len(values)):
                    ...
                    if hasarray:
                        ...
                        #an integer or float
                        if float(thevalue) == 0 and m > 0 and len(operations) >= m and operations[m-1] == "/DIV/":
