...
def create_desi_spectro_kdtree(infn, outfn, racol='ra', deccol='dec', healpix=True):
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        concat_fns.append('%s/ekd-0[12345]' % tempdir)
        ...
        if not healpix:
            ...
            rtn = os.system(cmd)
            assert(rtn == 0)
            concat_fns.append('%s/tfkd-0[12345]' % tempdir)

        # Concatenate all the HDUs!
        cmd = 'cat %s %s > %s' % (sfn, ' '.join(concat_fns), outfn)
        print(cmd)
        rtn = os.system(cmd)
        assert(rtn == 0)
